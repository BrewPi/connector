...
class BaseControllerAsyncLogTest(unittest.TestCase):
    ...
    def test_async_log_set_on_object(self):
        ...
        sut.handle_async_log_values((LongEncoder().encode(123456), [1], [([2], b'\x10\x20' )] ))
        # the decode method is called, and the return value from that used to call _update_value
        ...
        assert_that(log_event, has_length(2), "expected each event to be a tuple of length 2 (time, list)")
        assert_that(log_event[0], is_(123456), "expected index 0 to be time")
        assert_that(self.log_events[0][1], has_length(1), "expected index 1 to be a list of log values")
        log_value = self.log_events[0][1][0]
        assert_that(log_value[0], is_(v), "expected the log value to reference object v")
        assert_that(log_value[1], is_(1234), "expected the log value to show the value 1234")
    ...
    def test_async_log_set_on_multiple_objects(self):
        ...
        sut.handle_async_log_values((LongEncoder().encode(123456), [1], [([2], b'\x10\x20' ), ([4], b'\x54\x32' )] ))
        # the decode method is called, and the return value from that used to call _update_value
        v.decode.assert_called_with(b'\x10\x20')
